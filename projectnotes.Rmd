...
df <- read_csv("songs_normalize.csv") %>%
  ...
  select(artist, song, explicit, year, popularity, speechiness, tempo, loudness, acousticness, valence)
df
...
## 4. songs per artist top 50 (bar chart)  - shiny app App2
...
ggplot(df_grouped, aes(x = reorder(artist, -song), y = song, fill = artist)) +
  ...
  labs(title = "Top 30 Artists with Most Spotify Top Tracks", x = "Artist", y = "Total Songs") +
...
## 3.2 Distribution of Song Speechiness
...
## 3.2 Distribution of Song Tempo
...
p <- p + transition_states(states = tempo,
                           transition_length = 2,
                           state_length = 1)
...
## 3.3 Distribution of Song Loudness
...
p <- ggplot(df, aes(x=loudness)) +
  geom_histogram(binwidth=0.7, fill="#BA110C", color="black") +
  labs(x="Loudness", y="Count", title="Distribution of Song Loudness")

p <- p + transition_states(states = loudness,
                           transition_length = 2,
                           state_length = 1)
...
## 3.4 Distribution of Song Acousticness
```{r, message = FALSE, echo = FALSE}
library(ggplot2)
library(ggplot2)
library(gganimate)
...
p <- ggplot(df, aes(x=acousticness)) +
...
p <- p + transition_states(states = acousticness,
                           transition_length = 2,
                           state_length = 1)
...
## 3.5 Distribution of Song Valence
```{r, message = FALSE, echo = FALSE}
library(ggplot2)
library(ggplot2)
library(gganimate)
